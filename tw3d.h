/*
	...
	Hastasına Bilgi:
	- Çalışırken King's Field 2 - West Shore dinliyorum
	...
	TODO:
	- Texture işini rahatlaştıracağım. stb iyi bir seçenek
...
TwInstance* twCreateInstance(const TwInstanceDesc* desc);
TwBoolean twDeleteInstance(TwInstance* inst);

/* === WINDOW === */

// şuanlık pencere yaratma ve context açma, atama içinde bulunuyor
TwBoolean twOpenWindow(TwInstance* inst);
TwBoolean twIsWindowActive(TwInstance* inst);
TwBoolean twCloseWindow(TwInstance* inst);

void twSwapBuffer(TwInstance* inst);
void twSwapInterval(TwInstance* inst, TwInt interval);
void twWait(TwUint msec); // bu konuda şüphelerim var

TwBoolean twKeyDown(TwInstance* inst, TwByte key);
