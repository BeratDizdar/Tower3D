...
TW3D_API void twSetFog(TwInstance* inst, TwVec3* color, TwFloat density) {
	glFogfv(TW_FOG_COLOR, (TwFloat[4]) { color->x, color->y, color->z, 1 });
	glFogf(TW_FOG_DENSITY, density);
}
TW3D_API void twSetWorldLight(TwInstance* inst, TwVec3* ambient) {
	glLightModelfv(TW_LIGHT_MODEL_AMBIENT, (TwFloat[4]) { ambient->x, ambient->y, ambient->z, 1.0f });
}
TW3D_API void twSetLight(TwInstance* inst, TwSizei light_id, TwVec3* pos, TwVec3* diffuse, TwVec3* ambient) {
	glLightfv(light_id, TW_POSITION, (TwFloat[4]) { pos->x, pos->y, pos->z, 1.0f }); // point light imiş
	glLightfv(light_id, TW_DIFFUSE, (TwFloat[4]) { diffuse->x, diffuse->y, diffuse->z, 1.0f });
	glLightfv(light_id, TW_AMBIENT, (TwFloat[4]) { ambient->x, ambient->y, ambient->z, 1.0f });
...
TW3D_API void twInterleavedArrays(TwInstance* inst, TwUint format, const void* pointer) {
	glInterleavedArrays(format, 0, pointer);
	inst->gpu_call_count++;
}
