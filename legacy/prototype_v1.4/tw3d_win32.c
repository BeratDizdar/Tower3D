...
typedef struct TwInstance {
	...
	// belki view ve proj matrixi ekleyebilirim buraya
	TwMat4 proj_mat, view_mat; // ve ekledim

	// resource
	TwUint* tex_pool;
	TwUint tex_count;
	TwUint tex_cap;
	TwUint active_tex; // Aktif dokuuuuuu, sen niye daha önceden aklıma gelmedin
	...
	// fn ptr
	void(*glDeleteProgramsARB)(TwInt, const TwUint*);
	void(*glGenProgramsARB)(TwInt, TwUint*);
	void(*glBindProgramARB)(TwUint, TwUint);
	void(*glProgramStringARB)(TwUint, TwUint, TwInt, const void*);
	void(*glActiveTextureARB)(TwUint);
...
TW3D_API void twMathRotate(TwMat4* out_mat, TwFloat angle_deg, const TwFloat* axis) {
	TwFloat rad = angle_deg * (TW_PI / 180.0f);
	TwFloat c = cos(rad);
	TwFloat s = sin(rad);
	TwFloat t = 1.0f - c;

	TwFloat length = sqrt(axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]);
	TwFloat x = axis[0], y = axis[1], z = axis[2];
	if (length > 0.0f) {
...
TW3D_API void twMathTranslate(TwMat4* out_mat, const TwFloat* xyz) {
	twMathIdentity(out_mat);
...
TW3D_API void twMathScale(TwMat4* out_mat, const TwFloat* xyz) {
	twMathIdentity(out_mat);
	out_mat->m[0] =  xyz[0];
	out_mat->m[5] =  xyz[1];
	out_mat->m[10] = xyz[2];
...
TW3D_API void twMathTransform(TwMat4* out_mat, const TwFloat* pos, TwFloat angle, const TwFloat* axis, const TwFloat* scale) {
	TwMat4 t_mat, r_mat, s_mat, temp_mat;
	twMathTranslate(&t_mat, pos);
	twMathRotate(&r_mat, angle, axis);
	twMathScale(&s_mat, scale);
	twMathMultiply(&temp_mat, &r_mat, &s_mat);
	twMathMultiply(out_mat, &t_mat, &temp_mat);
}

TW3D_API TwFloat twMathLerp(TwFloat v0, TwFloat v1, TwFloat t) {
	return v0 + t * (v1 - v0);
}
...
TW3D_API TwBoolean twDeleteInstance(TwInstance* inst) {
	if (inst->tex_cap > 0) {
		...
		printf("[TW3D] : [+] : glDeleteTextures\n");
	}
	free(inst->tex_pool);
	if (inst->shader_cap > 0) {
		inst->glDeleteProgramsARB(inst->shader_count, inst->shader_pool);
		printf("[TW3D] : [+] : glDeleteProgramsARB\n");
	}
	free(inst->shader_pool);
	if (inst->model_cap > 0) {
		for (int i = 0; i < inst->model_count; i++) {
			free((void*)inst->model_pool[i].data);
		}
		free(inst->model_pool);
		printf("[TW3D] : [+] : free(models)\n");
	}
	if (inst != NULL) {
		inst->is_active = TW_FALSE;
		printf("[TW3D] : [+] : free(instance)\n");
		free(inst);
		return TW_TRUE;
	}
	return TW_FALSE;
...
TW3D_API TwBoolean twInstanceAllocator(TwInstance* inst, TwDictionary type, TwUint count) {
	switch (type) {
	case TW_ALLOC_POOL_TEX:
		inst->tex_pool = (TwUint*)malloc(count * sizeof(TwUint));
		if (inst->tex_pool == NULL) return TW_FALSE;
		inst->tex_cap = count;
		inst->tex_count = 0;
		break;
	case TW_ALLOC_POOL_SHADER:
		inst->shader_pool = (TwUint*)malloc(count * sizeof(TwUint));
		if (inst->shader_pool == NULL) return TW_FALSE;
		inst->shader_cap = count;
		inst->shader_count = 0;
		break;
	case TW_ALLOC_POOL_MODEL:
		inst->model_pool = (TwT3D*)malloc(count * sizeof(TwT3D));
		if (inst->model_pool == NULL) return TW_FALSE;
		inst->model_cap = count;
		inst->model_count = 0;
		break;
...
TW3D_API TwBoolean twOpenWindow(TwInstance* inst) {
	...
	//if (strstr(ext, "GL_ARB_vertex_program")) printf("[TW3D] ARB online\n");

	TwInt max_cmd, max_reg;
	((void(*)(TwUint, TwUint, TwInt*))wglGetProcAddress("glGetProgramivARB"))(TW_FRAGMENT_PROGRAM_ARB, TW_MAX_PROGRAM_INSTRUCTIONS_ARB, &max_cmd);
	((void(*)(TwUint, TwUint, TwInt*))wglGetProcAddress("glGetProgramivARB"))(TW_FRAGMENT_PROGRAM_ARB, TW_MAX_PROGRAM_TEMPORARIES_ARB, &max_reg);
	printf("[TW3D] max ARB line: %d\n", max_cmd);
	printf("[TW3D] max temp register: %d\n", max_reg);

	glEnable(GL_DEPTH_TEST);
	if (inst->culling == TW_TRUE) {
		glEnable(GL_CULL_FACE);
		glCullFace(GL_BACK);
	}
	glEnable(TW_FRAGMENT_PROGRAM_ARB);
	inst->glDeleteProgramsARB = wglGetProcAddress("glDeleteProgramsARB");
	inst->glGenProgramsARB = wglGetProcAddress("glGenProgramsARB");
	inst->glProgramStringARB = wglGetProcAddress("glProgramStringARB");
	inst->glBindProgramARB = wglGetProcAddress("glBindProgramARB");
	inst->glActiveTextureARB = wglGetProcAddress("glActiveTextureARB");
	inst->gpu_call_count += 2;
}
...
TW3D_API void twLoadModelViewMatrix(TwInstance* inst, const TwMat4* view, const TwMat4* model) {
	TwMat4 modelview_mat;
	twMathMultiply(&modelview_mat, view, model);
	glMatrixMode(GL_MODELVIEW);
	glLoadMatrixf(modelview_mat.m); inst->gpu_call_count += 2;
}

TW3D_API void twInitCamera(TwInstance* inst, TwFloat fov, TwFloat aspect, TwFloat near_val, TwFloat far_val) {
	twMathPerspective(&inst->proj_mat, fov, aspect, near_val, far_val);
	twLoadProjectionMatrix(inst, &inst->proj_mat);
}
TW3D_API void twCameraLookAt(TwInstance* inst, TwFloat* eye, TwFloat* center, TwFloat* up) {
	twMathLookAt(&inst->view_mat, &(TwVec3){ eye[0], eye[1], eye[2] }, & (TwVec3) { center[0], center[1], center[2] }, & (TwVec3) { up[0], up[1], up[2] });
}
TW3D_API void twCameraView(TwInstance* inst, TwMat4* out_mat) {
	*out_mat = inst->view_mat;
}
...
TW3D_API void twInterleavedArrays(TwInstance* inst, TwUint format, const void* pointer) {
	glInterleavedArrays(format, 0, pointer); inst->gpu_call_count++;
}

TW3D_API void twDrawMesh(TwInstance* inst, TwID texture_id, TwUint format, TwUint mode, TwUint count, const void* pointer) {
	twBindTextureIndex(inst, texture_id);
	twInterleavedArrays(inst, format, pointer);
	twDrawArrays(inst, mode, 0, count);
}

TW3D_API void twDrawModel(TwInstance* inst, TwID model) {
	twInterleavedArrays(inst, TW_T2F_N3F_V3F, &inst->model_pool[model].data[0]);
	twDrawArrays(inst, inst->model_pool[model].mode, 0, inst->model_pool[model].vertex_count);
}

/* SHADER */

TW3D_API TwID twLoadShaderARB(TwInstance* inst, const char* source) {
	if (inst->shader_count >= inst->shader_cap) return TW_ID_ERROR;

	TwUint gl_id;
	inst->glGenProgramsARB(1, &gl_id);
	inst->glBindProgramARB(TW_FRAGMENT_PROGRAM_ARB, gl_id);
	inst->glProgramStringARB(TW_FRAGMENT_PROGRAM_ARB, TW_PROGRAM_FORMAT_ASCII_ARB, strlen(source), source);
	inst->gpu_call_count += 3;
	inst->shader_pool[inst->shader_count] = gl_id;
	TwID index = inst->shader_count;
	inst->shader_count++;

	printf("[TW3D] : [+] : Shader Loaded, Index: %d\n", index);
	return index;
}

TW3D_API TwID twLoadShaderFileARB(TwInstance* inst, const char* filepath) {
	...
	fclose(f);
	TwID shader_id = twLoadShaderARB(inst, buffer);
	free(buffer);
	return shader_id;
}

TW3D_API void twBindShaderARB(TwInstance* inst, TwID shader) {
	if (shader < 0 || shader >= inst->shader_count) return;

	TwUint gl_id = inst->shader_pool[shader];
	if (inst->active_shader == gl_id) return;
	inst->glBindProgramARB(TW_FRAGMENT_PROGRAM_ARB, gl_id);
	inst->active_shader = gl_id;
	inst->gpu_call_count++;
}
TW3D_API void twActiveTextureARB(TwInstance* inst, TwARB texture) {
	inst->glActiveTextureARB(texture);
	inst->gpu_call_count++;
}
...
TW3D_API void twBindTextureIndex(TwInstance* inst, TwID texture_id) {
	if (inst->active_tex == inst->tex_pool[texture_id]) return;
...
TW3D_API TwID twLoadT3D(TwInstance* inst, const char* filepath) {
	if (inst->model_count >= inst->model_cap) return TW_ID_ERROR;

	FILE* f = fopen(filepath, "r");
	if (!f) {
		printf("[TW3D] : [-] : HATA! Model bulunamadi: %s\n", filepath);
		return -1;
	}

	char type_buffer[16];
	TwUint vertex_count;
	fscanf(f, "%15s %u", type_buffer, &vertex_count);

	TwUint draw_mode = TW_QUADS;
	if (strcmp(type_buffer, "TRIANGLE") == 0 || strcmp(type_buffer, "TRIANGLES") == 0) {
		draw_mode = TW_TRIANGLES;
	}

	TwFloat* raw_data = (TwFloat*)malloc(sizeof(TwFloat) * vertex_count * 8);
	if (!raw_data) {
		fclose(f);
		return -1;
	}

	for (TwUint i = 0; i < vertex_count; i++) {
		for (int j = 0; j < 8; j++) {
			fscanf(f, "%f", &raw_data[i * 8 + j]);
		}
	}
	fclose(f);

	TwID index = inst->model_count;
	inst->model_pool[index].data = raw_data;
	inst->model_pool[index].vertex_count = vertex_count;
	inst->model_pool[index].mode = draw_mode;
	inst->model_count++;

	printf("[TW3D] : [+] : Model Loaded (Index: %d, Type: %s, Verts: %u)\n", index, type_buffer, vertex_count);
	return index;
}