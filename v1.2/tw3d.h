/*
	...
	- Kendi implementasyonum render için OpenGL 1.1 kullanıyor. Açıkçası çoğunlukla isimleri
	sarmalıyorum. Her ne kadar OpenGL'e bayılmasamda basitliği benim için oldukça cezbedici.
	Fakat geliştirdikçe farklı şeyler deneyebilirim.
	- Elimden geldiğince en işe yarabilecek sistemi yaratmaya çalışıyorum. Bu yolda bazı eski
	fonksiyonları sonraki sürümlere taşımaya devam edebilirim. Bunun tek sebebi alternatif
	yaratmak. Ama tatmin olduğum yapıya ulaştığımda sadeleştirmeye gideceğim.
	- Yani temeldeki amacım olabildiğince az GPU call kullanarak çalışmak. Bu yolda biraz
	şişirebilirim burayı fakat dediğim gibi niyetim kullanım basitliği ile beraber güçlü bir API.
	...
	- Başka bir implementasyon olacak mı?
		- Biraz vaktim olursa vulkan veya d3d yönünde bir çalışma olabilir.
	...
	TODO:
	- SetFeatures için akıllı filtreleme lazım
	- Kendi fonksiyonlarımı da kırpmam lazım
	- VRAM'e buffer gönderme işini basitleştireceğim.
	- Ses eklemek iyi olabilir.
	- SIMD fonksiyonları iyi bir seçenek gibi.
	- Thread işi ekleyebilirim (Renderer hariç) [THREAD_SAFE makrosu]
	- Dümdüz sarmalamaları azaltıp, işlevi arttırabilirim.
	- Pencere işlevleri artmalı
	- Input sayısı artmalı
	- Debug layer???
	- Shader???
	- VRAM parselleme???
...
THREAD_SAFE void twMathRotate(TwMat4* out_mat, TwFloat angle_deg, const TwVec3* axis);
THREAD_SAFE void twMathMultiply(TwMat4* out_mat, const TwMat4* mat_a, const TwMat4* mat_b);
THREAD_SAFE void twMathTranslate(TwMat4* out_mat, const TwVec3* xyz);
THREAD_SAFE void twMathScale(TwMat4* out_mat, const TwVec3* xyz);
THREAD_SAFE void twMathTransform(TwMat4* out_mat, const TwVec3* pos, TwFloat angle, const TwVec3* axis, const TwVec3* scale);
...
/* === RENDER === */
	// FEATURE & ENVIRONMENT
		// LEGACY
		void twEnable(TwInstance* inst, TwUint cap);
		void twDisable(TwInstance* inst, TwUint cap);
		...
		void twAlphaFunc(TwInstance* inst, TwUint func, TwFloat ref);
		...
		void twCullFace(TwInstance* inst, TwUint mode);
		void twFrontFace(TwInstance* inst, TwUint mode);
		void twLightModelf(TwInstance* inst, TwUint pname, TwFloat param);
		void twLightModelfv(TwInstance* inst, TwUint pname, const TwFloat* params);
		void twLightf(TwInstance* inst, TwUint light, TwUint pname, TwFloat param);
		void twLightfv(TwInstance* inst, TwUint light, TwUint pname, const TwFloat* params);
		void twMaterialf(TwInstance* inst, TwUint face, TwUint pname, TwFloat param);
		void twMaterialfv(TwInstance* inst, TwUint face, TwUint pname, const TwFloat* params);
		void twShadeModel(TwInstance* inst, TwUint mode);
		void twColorMaterial(TwInstance* inst, TwUint face, TwUint mode);
		void twFogf(TwInstance* inst, TwUint pname, TwFloat param);
		void twFogfv(TwInstance* inst, TwUint pname, const TwFloat* params);
		void twFogi(TwInstance* inst, TwUint pname, TwUint param);
		void twTexEnvf(TwInstance* inst, TwUint target, TwUint pname, TwFloat param);
		void twTexEnvfv(TwInstance* inst, TwUint target, TwUint pname, const TwFloat* params);
		void twHint(TwInstance* inst, TwUint target, TwUint mode);
		...
		// NEW
		void twSetFeatures(TwInstance* inst, TwFeature mask); // bunda gpu call sayım hatası var unutma
		void twSetFog(TwInstance* inst, TwVec3* color, TwFloat density);
		void twSetWorldLight(TwInstance* inst, TwVec3* ambient);
		void twSetLight(TwInstance* inst, TwSizei light_id, TwVec3* pos, TwVec3* diffuse, TwVec3* ambient);

	// ASSET PIPELINE
		// TEXTURE
			// LEGACY
			void twBindTexture(TwInstance* inst, TwUint target, TwUint texture);
			void twGenTextures(TwInstance* inst, TwSizei n, TwUint* textures);
			void twTexParameteri(TwInstance* inst, TwUint target, TwUint pname, TwInt param);
			void twTexImage2D(TwInstance* inst, TwUint target, TwInt level, TwInt internalformat, TwSizei width, TwSizei height, TwInt border, TwUint format, TwUint type, const void* pixels);
			void twDeleteTextures(TwInstance* inst, TwSizei n, const TwUint* textures);
			// NEW
	...
	void twLoadModelViewMatrix(TwInstance* inst, const TwMat4* m);

	// DRAW
	void twClear(TwInstance* inst, TwUint mask);
	void twTexCoordPointer(TwInstance* inst, TwInt size, TwUint type, TwSizei stride, const void* pointer);
	void twVertexPointer(TwInstance* inst, TwInt size, TwUint type, TwSizei stride, const void* pointer);
	void twColorPointer(TwInstance* inst, TwInt size, TwUint type, TwSizei stride, const void* pointer);
	void twNormalPointer(TwInstance* inst, TwUint type, TwSizei stride, const void* pointer);
	void twEnableClientState(TwInstance* inst, TwUint array);
	void twDisableClientState(TwInstance* inst, TwUint array);
