/*
	...
	- Oyun geliştirme yaparken geliştiricilere daha fazla alan sağlarken düzgün araçlar oluşturmak.
	...
	- Fixed-Function ve Programmable yöntemlerin ikisini de bulundurulmalı. (şimdilik)
	- İlerde tw3d_audio.h ekleyebilirim.
	...
	TODO:
	- Vertexleri toplu yollayacak bir sistem iyi olur
		- Tabi ARB için de
	- VRAM'e buffer gönderme işini basitleştireceğim.
	- Pencere işlevleri artmalı
	- Input işi önemli (gamepad işi de)
	- Debug layer???
	- VRAM parselleme???
	- FBO
...
/* === RENDER === */
	// FEATURE & ENVIRONMENT
		...
		void twSetFog(TwInstance* inst, TwFloat* color, TwFloat density);
		void twSetWorldLight(TwInstance* inst, TwFloat* ambient);
		void twSetLight(TwInstance* inst, TwSizei light_id, TwFloat* pos, TwFloat* diffuse, TwFloat* ambient);
	...
	void twInterleavedArrays(TwInstance* inst, TwUint format, const void* pointer);
		// NEW
		void twDrawMesh(TwInstance* inst, TwID texture_id, TwUint format, TwUint mode, TwUint count, const void* pointer);

	// SHADER
	TwID twLoadShaderARB(TwInstance* inst, TwInt type, const char* source);
	void twBindShaderARB(TwInstance* inst, TwInt type, TwID shader);
	void twUnbindShaderARB(TwInstance* inst, TwInt type);
	void twDeleteShaderARB(TwInstance* inst, TwInt n, const TwUint* programs);
