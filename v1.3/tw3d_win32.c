...
TW3D_API void twMathTransform(TwMat4* out_mat, const TwVec3* pos, TwFloat angle, const TwVec3* axis, const TwVec3* scale) {
...
TW3D_API void twSetFeatures(TwInstance* inst, TwFeature feature_mask) {
	...
	if (feature_mask & TW_FEATURE_LIGHTING) {
		glEnable(TW_LIGHTING);
		glColorMaterial(TW_FRONT, TW_AMBIENT_AND_DIFFUSE); // SAKLADIĞINI UNUTMA
	} 
	else glDisable(TW_LIGHTING);
	if (feature_mask & TW_FEATURE_BLEND) glEnable(TW_BLEND);
	else glDisable(TW_BLEND);
	if (feature_mask & TW_FEATURE_FOG) {
		glEnable(TW_FOG);
		glFogi(TW_FOG_MODE, TW_EXP2);    // | BUNLARI SAKLADIĞINI UNUTMA
		glHint(TW_FOG_HINT, TW_FASTEST); // |
	}
	...
	if (feature_mask & TW_FEATURE_COLOR_MATERIAL) {
		glEnable(TW_COLOR_MATERIAL);
		glAlphaFunc(TW_GREATER, 0.1f); // | SAKLADIĞINI UNUTMA
	}
	else glDisable(TW_COLOR_MATERIAL);
	if (feature_mask & TW_FEATURE_CULL_FACE) {
		glEnable(TW_CULL_FACE);
		glCullFace(TW_BACK); // SAKLADIĞINI UNUTMA
	}
...
TW3D_API void twSetFog(TwInstance* inst, TwFloat* color, TwFloat density) {
	glFogfv(TW_FOG_COLOR, color);
	glFogf(TW_FOG_DENSITY, density);
}
TW3D_API void twSetWorldLight(TwInstance* inst, TwFloat* ambient) {
	glLightModelfv(TW_LIGHT_MODEL_AMBIENT, ambient);
}
TW3D_API void twSetLight(TwInstance* inst, TwSizei light_id, TwFloat* pos, TwFloat* diffuse, TwFloat* ambient) {
	glLightfv(light_id, TW_POSITION, pos); // point light imiş
	glLightfv(light_id, TW_DIFFUSE, diffuse);
	glLightfv(light_id, TW_AMBIENT, ambient);
	glLightf(light_id, TW_CONSTANT_ATTENUATION, 1.0f);
	glLightf(light_id, TW_LINEAR_ATTENUATION, 0.02f);
	glLightf(light_id, TW_QUADRATIC_ATTENUATION, 0.08f);

	inst->gpu_call_count += 6;
}
...
TW3D_API TwID twLoadShaderARB(TwInstance* inst, TwInt type, const char* source) {
	TwID shader_id;
	((void(*)(TwInt, TwUint*))wglGetProcAddress("glGenProgramsARB"))(1, &shader_id);
	((void(*)(TwUint, TwUint))wglGetProcAddress("glBindProgramARB"))(type, shader_id);
	((void(*)(TwUint, TwUint, TwInt, const void*))wglGetProcAddress("glProgramStringARB"))(type, TW_PROGRAM_FORMAT_ASCII_ARB, strlen(source), source);
	inst->gpu_call_count += 3;
	return shader_id;
}

TW3D_API void twBindShaderARB(TwInstance* inst, TwInt type, TwID shader) {
	((void(*)(TwUint, TwUint))wglGetProcAddress("glBindProgramARB"))(type, shader);
	glEnable(type); // buraya düzenleme şart. hey inst ;)
	inst->gpu_call_count += 2;
}
TW3D_API void twUnbindShaderARB(TwInstance* inst, TwInt type) {
	glDisable(type); inst->gpu_call_count++;
}

TW3D_API void twDeleteShaderARB(TwInstance* inst, TwInt n, const TwUint* programs) {
	((void(*)(TwInt, const TwUint*))wglGetProcAddress("glDeleteProgramsARB"))(n, programs);
	inst->gpu_call_count++;
}
TW3D_API void twActiveTextureARB(TwInstance* inst, TwARB texture) {
	((void(*)(TwUint))wglGetProcAddress("glActiveTextureARB"))(texture);
	inst->gpu_call_count++;
}
